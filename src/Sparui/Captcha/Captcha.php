...
class Captcha
{
    ...
    public static function instance(){
        if(!self::$object){
            ...
            self::$object = new Captcha($config);
        }
    ...
    public function check($value) {
        ...
        if( $value != null && $captchaHash != null && Hash::make(Str::lower($value) == $captchaHash) ){
            return true;
        }
        return false;
    }
    ...
    public function create() {
        //生成指定长度字符串
        $_char = Str::random($this->length);

        //保存session
        Session::put('captchaHash',Hash::make(Str::lower($_char)));
        // 图片宽(px)
        ...
        // 验证码使用随机字体
        $ttfPath = __DIR__ . '/../../../public/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
    ...
    private function _writeNoise() {
        for($i = 0; $i < 10; $i++){
            ...
            for($j = 0; $j < 5; $j++) {
                // 绘杂点
                imagestring($this->_image, 5, mt_rand(-10, $this->imageW),  mt_rand(-10, $this->imageH), Str::random(1), $noiseColor);
            }
        }
    }
